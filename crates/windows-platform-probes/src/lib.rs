...
//! | [`queue_contention::measure`] | binary only | what the bounded array queue's contended tail claim costs against a plain `fetch_add`, an upper bound on what `reserving_mpsc`'s read of the consumer's position adds (the ratio is between two complete push paths, so it bounds that read rather than isolating it), and how the claim word's bit apportionments compare -- binary only because it wants more cores than a hosted runner has, takes about a minute, and reports the two shapes as equivalent unless built with `--release` |
#![cfg(windows)]
