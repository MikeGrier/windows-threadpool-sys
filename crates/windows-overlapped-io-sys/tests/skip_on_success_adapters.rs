...
#[cfg(feature = "device")]
mod device {
    use super::{NO_PACKET_TIMEOUT_MS, skip_on_success, temp_file_with};
    use windows_overlapped_io_sys::{CompletionPort, Started, UnassociatedEndpoint};

    /// `FSCTL_GET_COMPRESSION`, which reads a small fixed-size value from an
    /// already-open local file and is therefore about as likely to complete
    /// inline as any real control code.
    const FSCTL_GET_COMPRESSION: u32 = 0x0009_003C;
    /// The `USHORT` compression state the control code writes.
    const COMPRESSION_STATE_LEN: usize = 2;

    #[test]
    fn an_ioctl_on_a_skip_endpoint_reports_whichever_arm_the_io_manager_chose() {
        let path = temp_file_with(b"ioctl target", "ioctl");
