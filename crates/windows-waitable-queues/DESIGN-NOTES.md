...
gate alone does not catch it either. That silent degradation, not a compile error, is the real reason
this shape may not simply widen its word, and D-37's `const` assertion on `is_always_lock_free()`
exists to turn it back into a build failure.
