...
fn corpus() -> Vec<(&'static str, Vec<u8>)> {
    vec![
        ...
        ("four_byte_emoji", b("😀")),
        ("supplementary_min", b("\u{10000}")),
        ("supplementary_max", b("\u{10FFFF}")),
...
fn matrix_ordering_is_lexicographic_over_content_units() {
    ...
    //
    //  1. `owned.sort()` is the only use of `Wtf8String`'s `Ord` in this test.
    ...
    //  4. `expected.sort()` sorts by `Vec<u8>`'s `Ord` -- std's lexicographic
    //     order over the bytes. That is a different comparator from (1), and is
    ...
    // order chosen by `Wtf8String::Ord` is std's lexicographic byte order, which
    // is the contract named in the test. The clone is what pins both sides to
    // one multiset, leaving the order as the single free variable -- it is
