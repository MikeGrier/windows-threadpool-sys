...
| D-10 | FFI fit is `*const u16` (`windows-sys` `PCWSTR`). This is **not** an `HSTRING` analog (WinRT is out of scope). A high-level `windows`-crate `Param<PCWSTR>` impl is **scheduled** as [CHECKLIST.md](CHECKLIST.md) M7, a feature-gated seam so it imposes no dependency when off. |
| D-11 | **Zero dependencies** (`widestring` is prior art only). The portable core is `alloc`-friendly; `no_std` / `alloc`-only support is **scheduled** as [CHECKLIST.md](CHECKLIST.md) M8, gating the `std`-only `OsStr` interop behind a default `std` feature. |
