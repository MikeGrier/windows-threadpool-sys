...
| D-10 | FFI fit is `*const u16` (`windows-sys` `PCWSTR`). This is **not** an `HSTRING` analog (WinRT is out of scope). A high-level `windows`-crate `Param<PCWSTR>` impl is **scheduled** as [CHECKLIST.md](CHECKLIST.md) M8, a feature-gated seam so it imposes no dependency when off. |
