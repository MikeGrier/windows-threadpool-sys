...
| D-3 | v1 ships **only** the `Wtf16` encoding, exposed as `Wtf16String` = `WtfString<Wtf16>` and `Wtf16Str` = `WtfStr<Wtf16>`. The `Wtf8` arm -- a `u8`/WTF-8 storage variant whose encode/decode, comparison, and formatting semantics this crate defines, backed by a crate-owned `Vec<u8>` (its WTF-8 storage matches `OsString`'s but the arm is not built on `OsString`; the storage field `Vec<E::Unit>` cannot be specialized to a distinct nominal type per encoding) -- is a designed-in seam, **deferred**: unscheduled post-v1 horizon work ([CHECKLIST.md](CHECKLIST.md) `M-inf.1`), gated on nothing. |
