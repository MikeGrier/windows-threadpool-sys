...
| D-11 | **Zero dependencies** (`widestring` is prior art only). The portable core is `alloc`-friendly; `no_std` / `alloc`-only support is **implemented** (M9) behind a default-on `std` feature that gates the `std`-only `OsStr` interop. Its shape is [D-18](#decision-index). |
...
| D-18 | **`no_std` is the crate's baseline; `std` is an additive, default-on feature (M9).** The crate root is unconditionally `#![no_std]` with `extern crate alloc`, so storage, `str`/`String` conversions, the mutation surface and the FFI pointer surface all build on `alloc` alone. The `std` feature adds exactly one thing -- the Windows `OsStr`/`OsString` interop -- because `OsStr` lives in `std` and has no `alloc`-only equivalent (D-14); it also forwards to `windows-core`'s own `std` feature when that optional dependency is on. `std` is linked under `cfg(test)` (the tests are std-only) and under `cfg(doc)` (so intra-doc links to `std` items resolve even in an `alloc`-only documentation build). `no_std`-ness is proven by **building for a bare-metal target** (`thumbv7em-none-eabi`) in CI, not merely by `--no-default-features` on a host, which would still succeed if a `std` dependency crept back in; the same target build *fails* with the `std` feature on, so the gate is demonstrably not vacuous. See [The no_std baseline (D-18)](DESIGN-RATIONALE.md#the-no_std-baseline-d-18). |
