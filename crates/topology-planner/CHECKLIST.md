...
| MX2 locality and ownership | authorized | `EP-X2.5` is the last open item; `EP-R1.7.1` precedes anything after it |
...
**`EP-X2.5` is authorized as an offline behavioral experiment; all other execution remains paused
...
- [ ] **EP-X2.5** -- **Exercise scatter/gather and broadcast/branch/join constraints.**
  **Authorized standalone scope:** implement a separate fan-out path in
  `experiments/request-reply`, preserving its three existing experiments unchanged. Follow
  [DESIGN-NOTES.md](experiments/request-reply/DESIGN-NOTES.md) -> `RR-D9`. Results stay in
  memory with no durability claim; no startup benchmarking, hardware gate or performance
