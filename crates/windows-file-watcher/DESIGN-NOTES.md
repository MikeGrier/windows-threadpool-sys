...
| D-28 | **A fault is watcher state, not a queued item.** A fault report is control data generated on the cadence, so neither of D-29's throttles can apply to it: it must not be dropped (the watch would silently never recover) and must not block (deadlock). It needs no queue slot at all -- a watcher cannot be faulted twice concurrently, because a faulted watcher is not running -- so the report is one error code plus one bit latched on the watcher, allocated with it. Writing it cannot fail or block; delivery to the client happens when there is room. The same argument makes every `Desync` a **latch** rather than a queued item, generalising what D-11 already did for `QueueFull`: reporting that the queue filled cannot itself require queue space. |
| <a id="d-29"></a>D-29 | **A full queue is survived by throttling each producer away from the enqueue, never by blocking there.** Blocking at a full queue is a deadlock rather than backpressure: the writer would be an I/O completion holding a pool thread, and the client's drain may itself be a pool callback (the D-25 doorbell integration), so the cadence can block pool threads waiting for a drain that needs one. Instead each producer is throttled at a safe point. **Control** comes from request processing, so the monitor stops draining the request queue; backpressure then lands on the client's own `subscribe()` call on the client's own thread. **Observation** comes from the I/O completion, so the watcher **stops re-arming the read**; backpressure propagates into the kernel's own change buffer, which is a grace period rather than a loss. The re-arm is gated on the queue having *room* rather than the enqueue failing -- a completed read must always have somewhere to put its batch, and checking at arm time is what makes "nothing is dropped" true rather than aspirational. If the client drains in time nothing is lost at all; if not, the kernel overflows, which is the already-specified `Desync { Overflow }`. A consequence worth stating: `QueueFull` largely stops occurring, and the loss a client does see is genuine kernel overflow -- "the OS dropped changes", which is true -- rather than "the crate dropped changes because you were slow", which was a choice. |
| D-30 | **Every request produces a completion, carried on the notification queue.** The request queue previously had no completions at all, which made M3.5's requirement -- assert no delivery after cancellation completes -- unprovable, since a client could not observe that its cancellation had been processed. Completions ride the notification queue rather than a side channel so the ordering guarantee is **structural**: a `Cancelled { watch }` sitting in the same ordered stream means everything before it belongs to the live watch and nothing after it does, which is the same reason `Desync` rides the stream (D-12). Uniformly *every* request, not only those whose contract seems to need it: subscribing can fail **permanently** (D-22's `NotADirectory` and `InvalidPath` have no retry path), so a fire-and-forget subscribe would hand back a `Watch` that will never fire and never say so. |
