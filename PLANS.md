...
| [crates/wtf-string/CHECKLIST.md](crates/wtf-string/CHECKLIST.md) | in progress | `OsString`-shaped strings with native `u16` (WTF-16), conversion-free storage for Windows FFI: an encoding-generic core (`WtfString<E>`) shipping the `Wtf16` arm, always-terminated storage, portable `str`/`String` conversions, and Windows-only lossless `OsStr` interop. Six milestones (M1 → M6). | [crates/wtf-string/DESIGN-NOTES.md](crates/wtf-string/DESIGN-NOTES.md) |
