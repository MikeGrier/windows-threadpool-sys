...
|---|---|---|---|
| [CHECKLIST.md](CHECKLIST.md) | completed | M1: amplify PR #42's contract-specification findings (the ten gap categories) from `windows-file-watcher` into `windows-overlapped-io-sys` and `windows-ioring-sys`. The follow-on audits are each crate's own milestone. | [DESIGN-NOTES.md](DESIGN-NOTES.md#specifying-a-delivery-contract) |
