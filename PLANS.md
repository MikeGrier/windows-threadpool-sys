...
| [CHECKLIST-review-baseline.md](CHECKLIST-review-baseline.md) | in progress | M1: close the gap that let an automated PR review on #46 raise seven false "`size_of` is not in scope" findings. The workspace's edition and MSRV are structurally invisible in a diff (the pinned toolchain file never changes, the root manifest's `[workspace.package]` table falls outside the members-list hunk, and the crate manifests only say `.workspace = true`), and pre-1.80 `size_of` call sites elsewhere in the workspace supply positive evidence for the wrong conclusion. States the baseline where a reviewer reads it, normalises the contradicting call sites, and guards the restatement against drift in CI. Feature-scoped and deleted when complete. | N/A |
